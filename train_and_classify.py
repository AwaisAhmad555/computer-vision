...
custom_features_dataset = pickle.dump(dataset_list,open("dataset\\custom_features_dataset.pkl", "wb"))

dataset_list = pickle.load(open("dataset\\custom_features_dataset.pkl", "rb"))

print(len(dataset_list))

#classifier = MLPClassifier(alpha=1e-5, hidden_layer_sizes=(200, 100, 100,), max_iter=1000,activation = 'relu',solver='adam',random_state=1)
...
"""feature_scaling_object = StandardScaler()
for i in range(len(array_new[:,1].tolist())):

    array_new[i,1] = feature_scaling_object.fit_transform(array_new[i,1])
